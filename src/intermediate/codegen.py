...
class CodeGen(object):
    ...
    def traverse(self, given_tree):
        if type(given_tree) is not tuple:
            return given_tree     
        elif given_tree[0] == 'statement':
            l = self.traverse(given_tree[1])
            r = self.traverse(given_tree[2])
            return '{}\n{}\n'.format(l,r)
        elif given_tree[0] == 'assignmentExpression':
            l = self.traverse(given_tree[2])
            r = self.traverse(given_tree[3])
            if type(r) is tuple:
    ...
    def generate(self):
        return self.traverse(self.tree)